...
#### Tasks can be requested to re-run selectively 

A new implicit `--rerun` option is now available for every task. The effect is similar to `--rerun-tasks`, but it forces a rerun only on the specific task to which it was directly applied. 
